...
- The follow-up runtime showed `bridge_ready=1` stable after warm-up, but visual residuals remained. The final backbuffer queue is distinct from the Streamline Compute queue, so publication alone was insufficient. The next revision inserts a targeted `consumer fence` wait on the exact final copy queue for the tray it reads; it does not wait on unrelated submissions.
