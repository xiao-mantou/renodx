...
- The first repair is deliberately DL2-local and FG-only. A new `AD FP16 -> PQ/RGB10 Bridge` mode recognizes only a recorded 0xAD original whose effective resource is FP16 and whose destination is a matching non-swapchain RGB10 tray. Inside Streamline's already-recording private copy command list it converts the FP16 Linear/BT.709 image at the fixed 203-nit reference into BT.2020/PQ in a RenoDX-owned render-target/copy-source bridge, copies that compatible bridge into the tray, then suppresses the incompatible original copy.
...
- Test after a full restart with Balanced + FG, HDR10, `ResourceUpgradeTest=33`, and `DLSS FG Final Color = AD FP16 -> PQ/RGB10 Bridge`. Success requires repeated `DL2 DLSS FG AD bridge ... rendered=1`, normal color, HDR headroom above 203 nit, and improved FG motion. Any `rendered=0`, black screen, or device removal invalidates the assumed copy-source/copy-destination resource states and the original copy remains the fallback only when bridge setup fails before command recording.
